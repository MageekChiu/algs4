 ...
 *
 * 可以排序，然后二分查找，O(nlogn).
 * 也可以用hash的方法，使得元素查找成为O(1)操作，这样整体就是O(n),
 * 但是有个问题就是重复元素比如{3,3} 6，需要处理一下
 * 最终 key -> 数组值，value-> index[+index...]
 * @author Mageek Chiu
    ...
    public static int[] twoSum(int[] nums, int target) {
        ...
        for (int i = 0; i < nums.length; i++) {
            if(map.containsKey(nums[i]))
                map.put(nums[i],map.get(nums[i])+","+i);//存入重复元素的index
            else
                map.put(nums[i], String.valueOf(i));//存入元素的index
        }
        for (int i = 0; i < nums.length; i++) {
            int other = target-nums[i];//nums[i]的补数
            if(other==nums[i]){//补数与本身相等
//                System.out.println(other+"+"+nums[i]);
                String[] r = map.get(other).split(",");
                if(r.length>1){// 确保 other 不是自身
                    res[0] = Integer.parseInt(r[0]);res[1]= Integer.parseInt(r[1]);
//                    System.out.println(r.length);
                    break;//因为题目结果唯一，所以可以直接返回了
                }else{// 否则是自身，不能算作结果，还得继续下一个循环
//                    System.out.println(other);
                    continue;
                }
            }
            if(map.containsKey(other)){
                res[0] = i;res[1]= Integer.parseInt(map.get(other));
//                System.out.println(res[0]+"+"+res[1]);
                if (res[0]>res[1]){// 交换顺序，保证小的在前面
                    res[0] = res[0]+res[1];
                    res[1] = res[0]-res[1];
                    res[0] = res[0]-res[1];
                }
    ...
    public static int[] twoSumSort(int[] nums, int target){
        ...
        System.arraycopy(nums,0,copy,0,len);//拷贝数组
        Arrays.sort(nums);//Java1.7之前的快排只是普通的快排，Java1.7的快排是一种双轴快排 DualPivotQuicksort顾名思义：双轴快排是基于两个轴来进行比较
        for (int i = 0; i < len; i++) {
            int other = target-nums[i];//nums[i]的补数
            int index =  BinarySearch.indexOf(nums,other);//二分查找
//            int index =  Arrays.binarySearch(nums,other);//二分查找
            if(index>0){
                res[0] = index > i ? i:index;
                res[1] = index > i ? index:i;
//                System.out.println(other+","+nums[i]+"---"+index+","+i+";"+res[0]+","+res[1]);
//                System.out.println(nums[res[0]]+","+nums[res[1]]);
//                System.out.println(copy[379]);
                break;
            }
        }
        int [] res1 = {0,0};
        res1[0] = BinarySearch.indexOf(copy,nums[res[0]]);
        res1[1] = BinarySearch.indexOf(copy,nums[res[1]]);
    ...
    // 1遍hash表的做法
    // 事实证明，我们可以一次完成。在进行迭代并将元素插入到表中的同时，直接进行判断，
    // 我们还会回过头来检查表中是否已经存在当前元素所对应的目标元素。
    // 即使当前漏了a,也会在后面检查a的补数时把a找回来。
    // 如果它存在，那我们已经找到了对应解，并立即将其返回。
    // 这是只返回一个解的做法,也可以返回全部解,也就是 twoSum1HashAllAnswer
    ...
    public static List<List<Integer>> twoSum1HashAllAnswer(int[] nums, int target) {
        Map<Integer, Integer> map = new HashMap<>();
        List<List<Integer>> res = new LinkedList<>();
        for (int i = 0; i < nums.length; i++) {
            int complement = target - nums[i];// 补数
            if (map.containsKey(complement)) {// 先判断
                List<Integer> thieRes = new LinkedList<>();// 一个解
                thieRes.add(map.get(complement));
                thieRes.add(i);// i后出现，所以后放i,保证结果都是按顺序的
//                System.out.println(thieRes+",target:"+target);
                res.add(thieRes);
            }
            map.put(nums[i], i);// 再放进去，这样就不会重复判断自己
        }
...
//        int[] nums = {3,3}; int target = 6;
        int[] nums = {3,3,2,5,4,2,2}; int target = 6;
//        int[] nums = {4,5,6,7,8,9,12,13,14,26,33,44}; int target = 27;
...
//        System.out.println(nums[379]+","+nums[632]);System.out.println(nums[633]+","+nums[633]);

//        long a = System.nanoTime();
//        int[] res = twoSum1Hash(nums,target);
//        long b = System.nanoTime();
//        int[] res1 = twoSumSort(nums,target);
//        long c = System.nanoTime();
//        System.out.println(res[0]+","+res[1]);
//        System.out.println(res[0]+","+res[1]+","+res1[0]+","+res1[1]+",时间对比："+(b-a)+"-"+(c-b));

        List<List<Integer>> resList = twoSum1HashAllAnswer(nums,target);
        for (List<Integer> res1 : resList) {
